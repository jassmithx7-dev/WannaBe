...
function layoutCornerToasts() {
  if (_pickToastEl) {
    _pickToastEl.style.left = 'auto';
    _pickToastEl.style.right = TOAST_EDGE + 'px';
    _pickToastEl.style.bottom = TOAST_EDGE + 'px';
  }
  if (_aiAlertEl) {
    var stackAbove = 0;
    if (_pickToastEl && _pickToastEl.isConnected) {
      stackAbove = _pickToastEl.offsetHeight + TOAST_GAP;
    }
    _aiAlertEl.style.left = 'auto';
    _aiAlertEl.style.right = TOAST_EDGE + 'px';
    _aiAlertEl.style.bottom = (TOAST_EDGE + stackAbove) + 'px';
  }
}
...
function showPickToast(pickNum, p) {
  ...
  if (_pickToastFadeTimer) clearTimeout(_pickToastFadeTimer);
  ...
  _pickToastTimer = setTimeout(function() {
    ...
    _pickToastFadeTimer = setTimeout(function() {
...
function showAIAlertPopup(text) {
  ...
  _aiAlertEl.style.cssText = 'position:fixed;z-index:9998;max-width:320px;background:rgba(45,32,8,.95);border:1px solid rgba(251,191,36,.35);border-radius:10px;padding:10px 14px;box-shadow:0 8px 24px rgba(0,0,0,.4);transition:opacity .4s ease,bottom .25s ease;opacity:1';
...
function renderBA(){
  ...
  if (filt === 'BESTFIT') {
    ...
    list = list.filter(function(p) { return !isDrafted(p) && hasScore(p) && !isAtPosLimit(p.pos); });
    var scored = list.map(function(p) { return scorePlayerForNeedFit(p, roster, rd); }).filter(Boolean);
...
function renderRoster(){
  ...
  var kCount = filled.filter(function(p) { return p.pos === 'K'; }).length;
  var defCount = filled.filter(function(p) { return p.pos === 'DEF'; }).length;
  var statsHtml =
  ...
  if (getPosLimit('K') != null) statsHtml += statCell('K', kCount);
  if (getPosLimit('DEF') != null) statsHtml += statCell('DEF', defCount);
...
function getBiggestNeedPos(count) {
  count = count || 1;
  if (myTeamIdx < 0) return count === 1 ? null : [];
  var roster = myRosterSlots.filter(Boolean);
  ...
  var ranked = ['QB', 'RB', 'WR', 'TE']
    .filter(function(pos) { return !isAtPosLimit(pos) && score[pos] > 0; })
    .sort(function(a, b) { return score[b] - score[a]; });
  if (count === 1) return ranked[0] || null;
...
function leagueRoundSummary(rd, curPick, po) {
  var roundEnd = rd * TEAMS;
  var picksLeft = 0;
  for (var pk = curPick; pk <= roundEnd && pk <= TEAMS * ROUNDS; pk++) {
    if (po && po[pk - 1] != null) picksLeft++;
  }
  var ts = window.leagueTendencies ? Object.values(window.leagueTendencies) : [];
  if (!ts.length) {
    return picksLeft > 0 ? picksLeft + ' pick' + (picksLeft !== 1 ? 's' : '') + ' left in Rd ' + rd : '';
  }
  var avg = function(arr) {
    var v = (arr || []).filter(function(x) { return x !== null && x !== undefined; });
    return v.length ? v.reduce(function(a, b) { return a + b; }, 0) / v.length : null;
  };
  var qbCount = ts.filter(function(t) {
    var q = avg(t.qbRounds);
    return q != null && rd >= Math.round(q) - 1 && rd <= Math.round(q) + 1;
  }).length;
  var teCount = ts.filter(function(t) {
    var q = avg(t.teRounds);
    return q != null && rd >= Math.round(q) - 1 && rd <= Math.round(q) + 1;
  }).length;
  var parts = [];
  if (picksLeft > 0) parts.push(picksLeft + ' left');
  if (qbCount >= 2) parts.push(qbCount + ' QB windows');
  if (teCount >= 2) parts.push(teCount + ' TE windows');
  if (!parts.length) return '';
  return 'Rd ' + rd + ': ' + parts.join(' · ');
}

var POS_LIMIT_SETTING_KEYS = {
  QB: ['max_qb', 'max_qbs', 'pos_limit_qb', 'position_limit_qb', 'limit_qb', 'qb_max', 'qb_limit', 'limit_pos_qb'],
  RB: ['max_rb', 'max_rbs', 'pos_limit_rb', 'position_limit_rb', 'limit_rb', 'rb_max', 'rb_limit', 'limit_pos_rb'],
  WR: ['max_wr', 'max_wrs', 'pos_limit_wr', 'position_limit_wr', 'limit_wr', 'wr_max', 'wr_limit', 'limit_pos_wr'],
  TE: ['max_te', 'max_tes', 'pos_limit_te', 'position_limit_te', 'limit_te', 'te_max', 'te_limit', 'limit_pos_te'],
  K: ['max_k', 'max_ks', 'pos_limit_k', 'position_limit_k', 'limit_k', 'k_max', 'k_limit', 'limit_pos_k'],
  DEF: ['max_def', 'max_defs', 'pos_limit_def', 'position_limit_def', 'limit_def', 'def_max', 'def_limit', 'limit_pos_def', 'max_dst', 'limit_dst', 'pos_limit_dst']
};

var POS_LIMIT_ALIASES = { qb: 'QB', rb: 'RB', wr: 'WR', te: 'TE', k: 'K', def: 'DEF', dst: 'DEF', d: 'DEF' };

function posFromLimitKey(key) {
  if (!key) return null;
  var lk = String(key).toLowerCase();
  var patterns = [
    /(?:max|limit|cap)[_-]?(qb|rb|wr|te|k|def|dst|d)s?\b/,
    /\b(qb|rb|wr|te|k|def|dst|d)s?[_-](?:max|limit|cap)\b/,
    /(?:pos|position)[_-]?(?:limit|max)[_-]?(qb|rb|wr|te|k|def|dst|d)s?\b/,
    /(?:pos|position)[_-]limit[_-](qb|rb|wr|te|k|def|dst|d)s?\b/
  ];
  for (var i = 0; i < patterns.length; i++) {
    var m = lk.match(patterns[i]);
    if (m) return POS_LIMIT_ALIASES[m[1].replace(/s$/, '')] || m[1].replace(/s$/, '').toUpperCase();
  }
  return null;
}
  ...
  sources.forEach(function(s) {
    ...
    if (s.position_limits && typeof s.position_limits === 'object') {
      Object.keys(s.position_limits).forEach(function(k) {
        var pos = POS_LIMIT_ALIASES[k.toLowerCase()] || k.toUpperCase();
        if (['QB', 'RB', 'WR', 'TE', 'K', 'DEF'].indexOf(pos) >= 0) setLimit(pos, s.position_limits[k]);
      });
    }
    Object.keys(s).forEach(function(k) {
      var pos = posFromLimitKey(k);
      if (pos && leaguePosLimits[pos] == null) setLimit(pos, s[k]);
    });
...
function getBestFitScoredPlayers() {
  var roster = myRosterSlots.filter(Boolean);
  var rd = Math.ceil(currentPick / TEAMS);
  var pool = players.filter(function(p) {
    return !p.drafted && !p.mockDrafted && p.customScore > 0 && !isAtPosLimit(p.pos);
  });
  var scored = pool.map(function(p) { return scorePlayerForNeedFit(p, roster, rd); }).filter(Boolean);
  scored.sort(function(a, b) { return b.score - a.score; });
  return scored;
}
...
function _scoreBySleeperRank() {
  ...
  const curves = {
    QB:  [530,510,492,476,461,447,434,422,410,399,388,378,368,358,348,338,328,318,308,298,288,278,268,258,248,238,228,218,210,202,194,186,178,170,163,156,149,142,136,130],
...
function startMockDraft() {
  ...
  // Pre-populate board with all keepers from all teams (source: savedPickLog covers both
  // setup-based keepers from buildKeeperPicks() and manual keepers from applyManualKeepers())
  mockState.savedPickLog.filter(function(l){return l.isKeeper;}).forEach(function(l) {
    var ti=l.teamIdx, pl=players.find(function(p){return p.name===l.player;});
    if(pl) pl.drafted=true;
    var mp=mockState.players.find(function(x){return x.name===l.player;});
    if(mp) mp.mockDrafted=true;
    pickLog.push({pick:l.pick,rd:l.rd,teamIdx:ti,team:teamNames[ti]||'T'+(ti+1),player:l.player,pos:l.pos,nfl:l.nfl,isKeeper:true});
    if(!teamRosters[ti]) teamRosters[ti]=[];
    teamRosters[ti].push(Object.assign({},pl||{name:l.player,pos:l.pos,team:l.nfl,customScore:0},{pickNum:l.pick,rd:l.rd,isKeeper:true}));
    if(!mockState.rosters[ti]) mockState.rosters[ti]=[];
    if(!mockState.rosters[ti].find(function(x){return x.name===l.player;}))
      mockState.rosters[ti].push(Object.assign({},pl||{name:l.player,pos:l.pos,team:l.nfl,customScore:0},{pickNum:l.pick,rd:l.rd,isKeeper:true}));
  });
...
function showDraftReport(rostersOverride, myTiOverride) {
  ...
  function getGrade(score, avg) {
    var d = (score - avg) / avg;
    if (d >= .15) return 'A+'; if (d >= .10) return 'A'; if (d >= .05) return 'A-';
    if (d >= .02) return 'B+'; if (d >= -.02) return 'B'; if (d >= -.05) return 'B-';
    if (d >= -.09) return 'C+'; if (d >= -.14) return 'C'; if (d >= -.20) return 'D';
    return 'F';
  }
  ...
  for (var ti = 0; ti < TEAMS; ti++) {
    ...
    roster.forEach(function(p) {
      ...
      if (pos[p.pos]) pos[p.pos].push(p);
    });
    summaries.push({ ti, name: teamNames[ti] || ('Team '+(ti+1)), total, pos });
  }

  var avg = summaries.reduce(function(s,t){ return s + t.total; }, 0) / TEAMS;
  summaries.sort(function(a,b){ return b.total - a.total; });

  // Projected record via round-robin probability
  function projRecord(myTotal) {
    var wins = summaries.reduce(function(s,t){
      return s + (t.total !== myTotal ? myTotal / (myTotal + t.total) : 0);
    }, 0);
    var w = wins / (TEAMS - 1) * 14;
    return { w: Math.round(w*10)/10, l: Math.round((14-w)*10)/10 };
  }

  // Top team at each position (by sum of top-N scores)
  function topAtPos(posKey, take) {
    var best = null, bestScore = 0;
    summaries.forEach(function(t) {
      var score = t.pos[posKey].slice().sort(function(a,b){ return (b.customScore||0)-(a.customScore||0); })
        .slice(0, take).reduce(function(s,p){ return s+(p.customScore||0); }, 0);
  ...
  html += '<div style="background:linear-gradient(135deg,#1a2f1a,#0e2a1a);border:1px solid #2ea043;border-radius:10px;padding:14px 20px;margin-bottom:18px;display:flex;align-items:center;gap:12px">'
    ...
    + '<div style="font-size:18px;font-weight:700;color:#e6edf3;margin-top:2px">'+champion.name+'</div>'
    + '<div style="font-size:11px;color:#7d8590;margin-top:2px">Score: '+Math.round(champion.total)+' · Grade: <span style="color:'+gradeColors[getGrade(champion.total,avg)]+'">'+getGrade(champion.total,avg)+'</span></div></div>'
    + '</div>';
  ...
  html += '<div style="display:grid;grid-template-columns:28px 1fr 48px 60px 72px;gap:8px;padding:8px 12px;font-size:10px;font-weight:700;color:#7d8590;text-transform:uppercase;letter-spacing:.05em;border-bottom:1px solid #21262d">'
    + '<span>#</span><span>Team</span><span style="text-align:center">Grade</span><span style="text-align:center">Score</span><span style="text-align:center">Proj W-L</span></div>';
  ...
  summaries.forEach(function(t, idx) {
    ...
    html += '<div style="display:grid;grid-template-columns:28px 1fr 48px 60px 72px;gap:8px;padding:8px 12px;border-bottom:1px solid #21262d;align-items:center;background:'+(isMe?'rgba(56,139,253,.06)':'transparent')+'">'
      ...
      + '<span style="font-size:12px;font-weight:'+(isMe?'700':'500')+';color:'+(isMe?'#93c5fd':isChamp?'#e6edf3':'#c9d1d9')+'">'
        +(isChamp?'🏆 ':'')+t.name+'</span>'
      + '<span style="text-align:center;font-size:12px;font-weight:700;color:'+gradeColors[grade]+'">'+grade+'</span>'
      + '<span style="text-align:center;font-size:11px;color:#9ca3af">'+Math.round(t.total)+'</span>'
      + '<span style="text-align:center;font-size:11px;color:#9ca3af">'+rec.w+' – '+rec.l+'</span>'
      + '</div>';
  });
  html += '</div>';
...
function renderNextPicksPanel() {
  ...
  if (roundEl) {
    var summary = leagueRoundSummary(curRd, curPick, owners);
  ...
  // Scan forward and collect up to 10 picks, noting where my pick falls
  var seq = [];
  for (var pk = curPick; pk <= total && seq.length < 10; pk++) {
    var ti = owners[pk - 1];
    if (ti === undefined) break;
    seq.push({ pk: pk, ti: ti });
  }

  var myIdx = myTi >= 0 ? seq.findIndex(function(s) { return s.ti === myTi; }) : -1;

  // Picks to render: up to 3 immediately before mine, mine, up to 4 after mine
  var beforeSlice = myIdx > 0  ? seq.slice(Math.max(0, myIdx - 3), myIdx)     : [];
  var afterSlice  = myIdx >= 0 ? seq.slice(myIdx + 1, myIdx + 5)               : seq.slice(0, 6);
  var myPickNum   = myIdx >= 0 ? seq[myIdx].pk : null;
  ...
  function buildCard(s, highlight) {
    var name   = teamNames[s.ti] || ('Team ' + (s.ti + 1));
    var roster = (rosters && rosters[s.ti]) ? rosters[s.ti] : [];
    var counts = { QB: 0, RB: 0, WR: 0, TE: 0 };
    roster.forEach(function(p) { if (counts[p.pos] !== undefined) counts[p.pos]++; });
    var rd     = Math.ceil(s.pk / TEAMS);
    var uid    = teamUserIds && teamUserIds[s.ti];
    var t      = window.leagueTendencies && ((uid && window.leagueTendencies[uid]) || window.leagueTendencies[name]);
  ...
  el.innerHTML = html || '<div style="color:#7d8590;font-size:11px;text-align:center;padding:16px 0">—</div>';
}
